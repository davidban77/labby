...
def _ip_query(v):
    ...
    if v.size > 1:
        ...
        if v.ip != v.network or not v.broadcast:
            return str(v.ip)
        # For the first IPv6 address in a network, netaddr will return it as a network address, despite it host address.
...
def _peer_query(v, vtype):
    if vtype == "address":
        ...
        if v.size == 4:
            if int(v.ip) % 4 == 0:
                raise ValueError("Network address of /30 has no peer")
            if int(v.ip) % 4 == 3:
                raise ValueError("Broadcast address of /30 has no peer")
            return str(netaddr.IPAddress(int(v.ip) ^ 3))
        raise ValueError("Not a point-to-point network")
...
def ipmath(value, amount):
    ...
    if not isinstance(amount, int):
        msg = (
            "You must pass an integer for arithmetic; " "{0} is not a valid integer"
        ).format(amount)
        raise ValueError(msg)
